...
export function actionGetTarifByCabType(response){    
	return {
        type: constants.GET_ALL_TARIFF,
        response
	};
...
export function gettarifbycabtype(cabtype) {        
    let url="http://localhost:4000/api/tarif/gettarifbycabtype/"+cabtype;
    return dispatch => {        
        return axios.get(url)		
        .then(response => 
        {            
            if(response){
                if(response.status === 200 && response.statusText === "OK"){                  
                    dispatch(actionGetTarifByCabType(response));
                    return response                
                }
                else{
                    // console.log("SignIn Failed Response::",response);                    
